...
Template.relatorio_geral.events({
    ...
    'submit form': function(e) {
                 e.preventDefault();
    var evento = $(e.target).find('#cbEvento').val()
   ...
   if(evento!==""){
           ...
           Session.set("noticias",Noticias.find({eventoId:evento,criacaoDt: {$gte: dataInicio, $lte: dataFim}},{sort: {criacaoDt: -1}}).fetch());
           Session.set("incidentes",Incidentes.find({eventoId:evento,criacaoDt: {$gte: dataInicio, $lte: dataFim}},{sort: {criacaoDt: -1}}).fetch());
           Session.set("consideracoes",Consideracoes.find({eventoId:evento,criacaoDt: {$gte: dataInicio, $lte: dataFim}},{sort: {criacaoDt: -1}}).fetch());
           Session.set("solicitacoes",Solicitacoes.find({eventoId:evento,criacaoDt: {$gte: dataInicio, $lte: dataFim}},{sort: {criacaoDt: -1}}).fetch());
   }else{
           ...
           Session.set("solicitacoes",Solicitacoes.find({criacaoDt: {$gte: dataInicio, $lte: dataFim}},{sort: {criacaoDt: -1}}).fetch());
   }
      ...
      'click #btntrazertudo': function(e) {
        ...
        Session.set("incidentes",Incidentes.find({},{sort: {criacaoDt: -1}}).fetch());
        Session.set("consideracoes",Consideracoes.find({},{sort: {criacaoDt: -1}}).fetch());
         Session.set("solicitacoes", Solicitacoes.find({},{sort: {criacaoDt: -1}}).fetch());
...
Template.relatorio_geral.helpers({
  ...
  solicitacoes: function(){
        ...
        return  Session.get("solicitacoes")
   },
   data: function(){
    var time = this.criacaoDt
    return moment(time).locale('pt-BR').format(' DD [de] MMMM YYYY, H:mm');
